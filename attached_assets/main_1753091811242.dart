...
class ScanBloc extends Bloc<ScanEvent, ScanState> {
  ...
  try {
    ...
    // Select files or directory depending on platform
...
//to open recycle bin folder from page
  static const platform = MethodChannel('com.example.dupfile_apk/open_folder');

Future<void> _openRecycleBinFolder(BuildContext context) async {
  try {
    final base = await getExternalStorageDirectory();
    final dupbin = Directory('${base!.path}/dupbin');
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Failed to open Recycle Bin: $e')),
    );
  }
}
...
Future<void> _restoreFile(BuildContext context, File file) async {
  ...
  try {
    ...
    _loadRecycledFiles(); // Refresh UI after restore
